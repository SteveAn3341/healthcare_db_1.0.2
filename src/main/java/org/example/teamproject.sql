...
select count(appointmentid) from appointments where status like "completed";
select count(recordid) from medicalrecords where  diagnosis   like "Asthma";
select sum(TotalAmount) from billing;
select sum(PaidAmount) from billing;
select sum(BalanceAmount) from billing where PaymentStatus like "unpaid";

select avg(TotalAmount) from billing;

select avg(datediff(current_date(), dateofbirth)/365) from patients;

select round(avg(PaidAmount),0) from billing where PaymentStatus like 'Paid';

select round(avg(dosage),2) from Prescriptions;

select min(appointmentdate) , max(appointmentdate) from appointments;
select min(totalamount) , max(totalamount) from billing;
select min(dosage) , max(dosage) from Prescriptions;
select FirstName as firstn , PatientID as id from (select (select FirstName , PatientID from patients where DateOfBirth = min(DateOfBirth)), min(dateofbirth)AS minage ,  max( dateofbirth) AS maxage from patients) as isdf ;

SELECT
    MIN(TIMESTAMPDIFF(YEAR, DateOfBirth, CURDATE())) AS YoungestAge,
    MAX(TIMESTAMPDIFF(YEAR, DateOfBirth, CURDATE())) AS OldestAge
FROM Patients;
...
SELECT totalAmount,
       CASE
           ...
           ELSE "OUT OF RANGE" END AS BILL_VALUE
FROM Billing;

SELECT FirstName, LastName, DateOfBirth,
       CASE
           WHEN (dateDiff(current_date(),dateofbirth)/365) < 18 THEN 'Minor'
           WHEN (dateDiff(current_date(),dateofbirth)/365) between 18 and 65 THEN 'Adult'
           WHEN (dateDiff(current_date(),dateofbirth)/365) > 65 THEN 'Senior'
           ELSE "OUT OF RANGE" END AS AgeGroup
FROM Patients;
...
SELECT * FROM Doctors
WHERE Specialization IN ('PEDIATRICS', 'CARDIOLOGY', 'Internal Medicine');

SELECT * FROM MedicalRecords
WHERE Diagnosis IN ('diabetes', 'hypertension', 'asthma');

SELECT * FROM Departments
WHERE DepartmentName IN ('surgery', 'cardiology', 'pediatricsDepartments');
